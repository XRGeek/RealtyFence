...
namespace Better.BuildInfo
{
    public sealed partial class BuildInfoWindow : EditorWindow, ISerializationCallbackReceiver
    {
        ...
        private GroupsInfo m_scenes = new GroupsInfo();
        ...
        private static void FilterAssets(List<AssetEntry> filteredAssets, List<AssetEntry> assets, FileSize minSize, FileSize? maxSize, Internal.BitArray categoriesMask, Internal.BitArray selectionMask, bool exclusiveSelection, string nameFilter, string[] propertiesNames)
        {
            ...
            foreach (var asset in assets)
            {
                ...
                if (!selectionMask.AndAny(asset.scenesMask))
                    continue;

                if (exclusiveSelection)
                {
                    if (asset.scenesMask.AndAnyNeg(selectionMask))
                        continue;
                }
            ...
            else if ( m_mode == Mode.UsedAssetsGrid || m_mode == Mode.UsedAssetsTreemap )
            {
                ...
                using (GUIHelpers.LabelWidth(60))
                {
                    ...
                    using (GUIHelpers.Horizontal())
                    {
                        ...
                        DoFilterControlGUI(m_scenes, m_skin.scenesFilterContent, () =>
                            {
                                using (GUIHelpers.Vertical(m_skin.header))
                                {
                ...
                var selectedSize = selectedAssets.Sum(x => x.size);
                ...
                {
                    {
                        GUILayout.Label(
                            string.Format("Selected <b>Size: {0}, Dependencies: {2}, Count: {1}</b>",
                                ...
                                selectedCount,
                                m_selectionDependencies.Sum(x => x.size).ToString()),
                            m_skin.richLabel);
                    }
                ...
                BottomPanel(panelRect, ref ui.dependenciesScroll, "Selection's Dependencies", m_selectionDependencies);

                panelRect.x += panelRect.width;
                BottomPanel(panelRect, ref ui.referencesScroll, "Assets Referencing Selection", m_selectionReferences);

                panelRect.x += panelRect.width;
                BottomPanel(panelRect, ref ui.scenesScroll, "Scenes Referencing Selection", m_selectionScenes);
            }
        }
        ...
        private void DoOverviewGUI()
        {
            using (GUIHelpers.ScrollView(ref ui.overviewScroll))
            {
                ...
                var allAssetsSize = m_categories.sizes.Sum(x => x);
                var allAssetsSizeDiff = m_categories.sizeDiffs.Sum(x => x);
        ...
        private void BottomPanel(Rect position, ref Vector2 scroll, string title, IList<AssetEntry> entries)
        {
            ...
            using (GUIHelpers.Vertical(GUI.skin.box))
            {
                ...
                GUIHelpers.FastScroll(ref scroll, entries.Count, Skin.LineHeight, null, position.height, index =>
                {
                    DrawEntryWithIcon(entries[index], EditorGUILayout.GetControlRect(), onClick: OnAssetEntryClicked);
                });
        ...
        private void AssetsTreemapGUI()
        {
            ...
            if (refreshTreeMap)
            {
                ...
                var rects = TreeMap.CalcMap(grouped.Select(x => x.Sum(xx => xx.size).KiloBytes), ui.lastTreeMapRect);
        ...
        {
            var sortInfo = m_sorts.FirstOrDefault(x => x.prop == property);
            ...
            closeButtonRect.x = closeButtonRect.xMax - 14.0f;
            closeButtonRect.width = 13.0f;
            closeButtonRect.height = 13.0f;
            closeButtonRect.y += ( rect.height - closeButtonRect.height ) / 2;
            ...
            if ( close == true || m_dragProperty == property )
            {
                if ( Event.current.type == EventType.Repaint )
                {
                    m_skin.toolbarButtonLeftAlignedStyle.Draw(rect, label, false, false, false, false);
                }
            }
            else if (GUI.Button(rect, label, m_skin.toolbarButtonLeftAlignedStyle))
            {
        ...
        private List<AssetEntry> Refresh(BuildInfo info, BuildInfo referenceInfo)
        {
            ...
            try
            {
                ...
                m_scenes = RefreshScenes(info, result);
        ...
        private GroupsInfo RefreshScenes(BuildInfo info, List<AssetEntry> entries)
        {
            var sortedScenes = info.scenes.OrderBy(x => x).ToArray();
            var sizes = new FileSize[sortedScenes.Length + 1];
            var counts = new int[sortedScenes.Length + 1];

            EditorUtility.DisplayProgressBar("Better Build Info", "Processing scenes...", 0);

            foreach (var assetEntry in entries)
            {
                // set the scenes mask
                assetEntry.scenesMask = new Internal.BitArray(sortedScenes.Length + 1, false);

                if (assetEntry.info.scenes.Count == 0)
                {
                    // unreferenced
                    assetEntry.scenesMask[sortedScenes.Length] = true;
                ...
                else
                {
                    foreach (var index in assetEntry.info.scenes.Select(x => Array.IndexOf(sortedScenes, x)))
                    {
                        ...
                        assetEntry.scenesMask[index] = true;
                        sizes[index] += assetEntry.size;
                        ++counts[index];
                    }
                }

                assetEntry.scenesMask[sortedScenes.Length] = !assetEntry.info.scenes.Any();
            }
